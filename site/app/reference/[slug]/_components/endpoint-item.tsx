import ParametersBox from "@common/docs/parameters-box";
...
export default async function EndpointItem<T extends ApiObject>({ object, method }: EndpointItemProps<T>) {
	...
	const { pathParameters, queryParameters, endpoint, endpointTitle, responseAttributes } = await import(
		...
		<>
			<div className="col-span-1 lg:col-span-1 flex flex-col gap-10 px-2">
				...
				<div className="flex flex-col gap-6">
					<ParametersBox parameters={pathParameters} type="path" />
					<ParametersBox parameters={queryParameters} type="query" />
					{responseAttributes && <ObjectAttributesBox title="Response Object" attributes={responseAttributes} />}
				</div>
			</div>
			<ItemGroup>
